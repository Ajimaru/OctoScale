...
ISO15693ErrorCode PN5180ISO15693::getSystemInfo(uint8_t *uid, uint8_t *blockSize, uint8_t *numBlocks,
                                                 uint8_t *dsfidOut, bool *dsfidPresentOut,
                                                 uint8_t *afiOut, bool *afiPresentOut,
                                                 uint8_t *icRefOut, bool *icRefPresentOut) {
  if (dsfidPresentOut) *dsfidPresentOut = false;
  if (afiPresentOut) *afiPresentOut = false;
  if (icRefPresentOut) *icRefPresentOut = false;
  ...
  if (infoFlags & 0x01) { // DSFID flag
    uint8_t dsfid = *p++;
    if (dsfidOut) *dsfidOut = dsfid;
    if (dsfidPresentOut) *dsfidPresentOut = true;
  ...
  if (infoFlags & 0x02) { // AFI flag
    uint8_t afi = *p++;
    if (afiOut) *afiOut = afi;
    if (afiPresentOut) *afiPresentOut = true;
  ...
  if (infoFlags & 0x08) { // IC reference
    uint8_t icRef = *p++;
    if (icRefOut) *icRefOut = icRef;
    if (icRefPresentOut) *icRefPresentOut = true;
    PN5180DEBUG("IC Ref=");
