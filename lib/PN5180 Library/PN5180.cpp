...
/*
 * MIFARE_AUTHENTICATE - 0x0C  (OctoScale patch, not in the upstream library)
 * Runs the MIFARE Classic Crypto1 authentication handshake for one sector, using the
 * PN5180's own crypto co-processor -- afterward, sendData()/readData() transceive
 * frames are transparently en/decrypted by the chip until the RF field is reset.
 * Command payload (12 bytes): BlockAddress(1) | AuthMode(1) | Key(6) | UID(4).
 * Response: 1 status byte (0 = authenticated OK; nonzero = failed, e.g. wrong key).
 */
// Command frame byte order (Cmd, Key(6), KeyType(1), Block(1), UID(4)) verified against
...
// Petrusevich) -- an initial guess at the layout (Cmd/Block/KeyType/Key/UID) silently
// produced a garbage authentication (the chip never ran the RF handshake, so the
// response byte was never actually written). No code was copied verbatim; only the
// wire-format byte ordering was cross-checked against that project's implementation.
bool PN5180::mifareAuthenticate(uint8_t blockAddr, const uint8_t *key, uint8_t authMode,
                                const uint8_t *uid, uint8_t *statusOut) {
  uint8_t full[13] = { PN5180_MIFARE_AUTHENTICATE,
                       key[0], key[1], key[2], key[3], key[4], key[5],
                       authMode, blockAddr,
                       uid[0], uid[1], uid[2], uid[3] };

  if (mifareAuthDebugCb) mifareAuthDebugCb(full, 13, nullptr, 0, false);

  uint8_t status = 0xFF;
  SPI.beginTransaction(PN5180_SPI_SETTINGS);
  bool ok = transceiveCommand(full, 13, &status, 1);
  SPI.endTransaction();

  if (mifareAuthDebugCb) mifareAuthDebugCb(full, 13, &status, 1, ok);
 ...
 */
// OctoScale patch (see /Volumes/Daten/Tinkering/OctoScale/lib/README-patch.md): the
// original BUSY-pin waits below have no timeout. A non-conforming/edge-case tag that
// never toggles BUSY as expected hangs this function forever -> on the ESP32 that
// starves the owning task and trips the Task Watchdog -> hard reboot. Each wait now
// bails out after PN5180_BUSY_TIMEOUT_MS and returns false (NSS is always deasserted
// before returning, so the SPI bus is never left wedged).
static const uint32_t PN5180_BUSY_TIMEOUT_MS = 50;

static inline bool pn5180WaitBusy(uint8_t busyPin, int level) {
  uint32_t t0 = millis();
  while (digitalRead(busyPin) != level) {
...
bool PN5180::transceiveCommand(uint8_t *sendBuffer, size_t sendBufferLen, uint8_t *recvBuffer, size_t recvBufferLen) {
  ...
  // 0.
  if (!pn5180WaitBusy(PN5180_BUSY, LOW)) return false;  // wait until busy is low
  // 1.
  ...
  // 3.
  if (!pn5180WaitBusy(PN5180_BUSY, HIGH)) { digitalWrite(PN5180_NSS, HIGH); return false; }  // wait until BUSY is high
  // 4.
  digitalWrite(PN5180_NSS, HIGH); delay(1);
  // 5.
  if (!pn5180WaitBusy(PN5180_BUSY, LOW)) return false;  // wait unitl BUSY is low
