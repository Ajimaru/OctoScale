...
public:
  ...
  /* cmd 0x0c -- MIFARE Classic Crypto1 authentication (OctoScale patch: NOT in the
   * upstream atrappmann/PN5180 library, needed for Mifare Classic block read/write).
   * authMode: 0x60=Key A, 0x61=Key B. key: 6 bytes. uid: the tag's 4-byte NFC-A UID
   * (single-size UID; 7-byte UIDs use their last 4 bytes here per the MF spec).
   * Returns true + *statusOut==0 on a successful authentication. */
  bool mifareAuthenticate(uint8_t blockAddr, const uint8_t *key, uint8_t authMode,
                          const uint8_t *uid, uint8_t *statusOut);

  // Optional debug hook (OctoScale patch): called once before send (recvBuf=null) and
  // once after (recvBuf set) with the raw frame bytes -- lets the project log via its
  // own debug console without this library needing to know about it. nullptr = off.
  typedef void (*MifareAuthDebugCb)(const uint8_t *sendBuf, size_t sendLen,
                                    const uint8_t *recvBuf, size_t recvLen, bool ok);
  static MifareAuthDebugCb mifareAuthDebugCb;
